...
def run_full_analysis(
    ...
    # 6. Tampering & Forensic Inspection (ELA, Noise, FFT Spectral, QR Signatures)
    tamper_res = detect_tampering(doc_img, doc_type)
    ...
    # Tampering & AI Synthetic Forensics
    if getattr(tamper_res, "isSynthetic", False):
        inconsistencies.append(f"AI-Generated Document: Synthetic frequency/texture signature detected (Confidence: {tamper_res.syntheticProbability:.0%})")
        risk_indicators.append(RiskIndicatorDto(
            type="SYNTHETIC_DOCUMENT_DETECTED",
            severity="HIGH",
            message=f"Document appears synthetically generated by diffusion/GAN models (Confidence: {tamper_res.syntheticProbability:.0%})"
        ))

    if tamper_res.detected:
        for r in tamper_res.reasons:
            if r not in inconsistencies:
